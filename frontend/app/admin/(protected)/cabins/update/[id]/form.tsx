  ...
  const router = useRouter();
    ...
    const handleFileUpload = (file: File | null) => {
        ...
        if (!validTypes.includes(file.type) || file.size > 1048576) {
          setImageError("Please upload a valid JPEG, JPG, or PNG file (max size: 1MB).");
          setImagePreview(null);
          setFormData((prev: any) => ({
            ...prev,
            service: { ...prev.service, imageUrl: "" },
          }));
        } else {
      ...
      const { getRootProps, getInputProps } = useDropzone({
        ...
        maxSize: 1048576,
        onDropRejected: () => {
          setImageError("File must be a JPEG, JPG, or PNG and less than 1MB.");
        },
      });
      ...
      {isMutating ? (
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <label>Service Name <span className={styles.required}>*</span></label>
            ...
            />
            {helperText.name && <small className={styles.error}>{helperText.name}</small>}
          </div>
          ...
          <label>Description <span className={styles.required}>*</span></label>
            ...
            />
            {helperText.description && <small className={styles.error}>{helperText.description}</small>}
          </div>
        </div>
          ...
          <div className={styles.capacityRateContainer}>
            <div className={styles.form_group}>
              ...
              />
              {helperText.minCapacity && <small className={styles.error}>{helperText.minCapacity}</small>}
            </div>
            ...
            <div className={styles.form_group}>
              ...
              <input
                ...
                onKeyDown={(e) => ["e", "E", "+", "-"].includes(e.key) && e.preventDefault()}
              />
              {helperText.maxCapacity && <small className={styles.error}>{helperText.maxCapacity}</small>}
            </div>
          ...
          <div className={styles.form_group}>
            ...
            <input
              ...
              onKeyDown={(e) => ["e", "E", "+", "-"].includes(e.key) && e.preventDefault()}
            />
            {helperText.price && <small className={styles.error}>{helperText.price}</small>}
          </div>
        </div>
          ...
          <div className={styles.additional_fees_container}>
            ...
            <div className={styles.form_group}>
              ...
              <input
                ...
                maxLength={50}
              />
              {helperText.additionalFeeType && <small className={styles.error}>{helperText.additionalFeeType}</small>}
            </div>
            ...
            <div className={styles.form_group}>
              ...
              {helperText.additionalFeeDescription && (
                <small className={styles.error}>{helperText.additionalFeeDescription}</small>
              )}
            </div>
        </div>
            ...
            <div className={styles.form_group}>
              ...
              />
              {helperText.additionalFeeAmount && <small className={styles.error}>{helperText.additionalFeeAmount}</small>}
              </div>
            </div>
          </div>
        </div>
